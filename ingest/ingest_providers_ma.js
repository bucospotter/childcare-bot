...
async function getData() {
  const src = process.env.MA_PROVIDERS_SRC || "";
  if (!src) throw new Error("Set MA_PROVIDERS_SRC to a local CSV/JSON path or an HTTP URL.");
  if (src.startsWith("http")) {
    const res = await fetch(src);
    if (!res.ok) throw new Error(`Fetch failed: ${res.status}`);
    const text = await res.text();
    // Heuristic: CSV if contains commas and newlines with header-like first row
...
function mapRecord(r) {
  // Map your CSV/JSON fields to our providers schema
  return {
...
async function main() {
  ...
  for (const raw of data) {
    ...
    await pool.query(
      ...
      ["MA", r.name, r.address, r.city, r.zip, r.license_type, r.license_status, r.qris_rating, r.source_url]
    );
    inserted++;
  }
  console.log(`✅ Inserted ${inserted} MA providers`);
  await pool.end();
}
